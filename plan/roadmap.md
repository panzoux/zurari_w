...
`phase6a-location-foundation` を fast-forward)。その後 6e.1〜6e.6 と 6c.5 も main に入った。
...
| help | **キーバインドの一覧がどこにも無い。** 「コマンドを足したらキーとヘルプ項目の両方を足す」という規約の後半が未実装で、Ctrl+B / Ctrl+D / Space / Apps は**発見不可能**だった。**ステータスバーの文脈ヒントは 6e.7 で入った**(並べ替え・隠しファイル・ピン留め、並べ替えモード中はその操作)。一覧としてのヘルプ画面は 6e.9 で、判断により後回し | `[~]` 6e.9 |
| R-1 | ~~Phase 6 のコミットが main に一つもマージされていない~~ → 6d 完了時点(6a〜6d/6f/6g、43コミット)を main へ fast-forward | `[x]` 解決(2026-09-05) |

## 見送った案

- **取り出し(eject)専用キー。** 6d.8 で `Ctrl+E` を一度実装したが撤回した。理由は二つ:
  `Ctrl+E` は Explorer の標準ではないので学習が必要で、しかも**取り出しはシェルのコンテキストメニューに
  既にある**(ドライブ行で Apps キー / 右クリック → 取り出し)。専用キーの利点は「速い」という一点だけで、
  そのためにキー空間を一つ使い、ヘルプに一行増やし、Core に `Msg`/`Effect`/`Entry.IsEjectable` を
  抱えることになる。**素早い操作のために再検討する価値はある**ので、キー体系をまとめて決めるとき
  (ステータスバーの文脈ヒント + ヘルプ画面)に他のコマンドと一緒に判断する。
  マウント(`.iso` を Enter で開く)は残っている — こちらはコンテキストメニューの `mount` に対応する
  「開く」操作そのもので、キーではなく既存の動作の自然な拡張だから。
...
- **PDF / Office の「中身が見える」プレビュー = `IPreviewHandler` のホスティング**(2026-09-12 記録)。
  Explorer のプレビューペインが PDF を出せるのは *サムネイル* ハンドラではなく **プレビューハンドラ**
  (`IPreviewHandler` `{8895b1c6-…}`)のため。この機の `.pdf` に登録されているのは "Microsoft PDF
  Previewer" `{3A84F9C2-…}` のみで、`IThumbnailProvider` は無い — 6c.6 のサムネイル経路で PDF が
  出ないのはこれが理由(phase6 記録 6c-8)。プレビューハンドラは HWND に描画するので画像として
  受け取れず、採用するならペインにその子ウィンドウをホストする方式になる。要調査はフォーカス・IME・
  DPI・ダークテーマの扱い。Phase 6 の範囲外。
...
| **ffmpeg の非同期起動**(現在は同期ブロッキング) | `[x]` 6c.1/6c.2 — 専用スロット + キャンセルでプロセスを kill |
| **サムネイルキャッシュ**(現在は毎回再生成) | `[x]` 6c.4 |
| Explorer のサムネイルを ffmpeg より先に(`Thumbs.db` を作らない) | `[x]` 6c.5 |
| シェル呼び出しを専用 STA スレッドへ(1件ずつ・最新優先・1.5秒バジェット・失敗も記憶) | `[x]` 6c.7 |
| ffmpeg と Explorer サムネイルの速度実測(2026-09-12、実ファイル 6本 849MB〜1.4GB) | ffmpeg が 6/6 で速い(205-624ms 対 282-722ms)。「外部ツール無しの方が安い」という 6c.5 の前提は崩れ、**ffmpeg 優先・シェルはフォールバック**に変更(記録 6c-10)|
| PDF / Office 文書のサムネイル | `[x]` 6c.6 — ただしハンドラ(Office・PDF リーダー等)が入っている環境のみ。この開発機には無く未実見 |
| 動画メタ(再生時間・解像度)の自前パース | `[ ]` 未着手(zrr は MP4 box を直接読む)。6c.6 でサムネイルの寸法を解像度として出すのをやめたため、解像度欄は現在空 |
